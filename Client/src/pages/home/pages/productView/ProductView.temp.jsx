  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        ...
        // Fetch category and seller info since they might not be populated in the API response
        let productData = response.data;
        
        // Fetch category name if it's not already populated
        if (productData.category && typeof productData.category === 'string') {
          try {
            const categoryResponse = await api.get(`/category/${productData.category}`);
            productData = {
              ...productData,
        ...
        // Prepare all product images including main image and additional images
        const images = [
        ...
        if (productData.variants && productData.variants.length > 0) {
          ...
          // Initialize selected attributes from the first variant
            ...
            <div className="product-details-section">
              ...
              <div className="product-details mt-8">
                ...
                <ul className="list-disc pl-5 space-y-1 text-gray-700">
                  <li>Category: {product.category?.name || 'Unknown'}</li>
                  <li>Seller: {product.seller?.name || 'Unknown'}</li>
                  <li>Availability: {selectedVariant?.stock > 0 ? 'In Stock' : 'Out of Stock'}</li>
                  {selectedVariant?.stock > 0 && <li>Stock: {selectedVariant.stock} available</li>}
                </ul>
              </div>
            </div>
          </div>
